...
def sample_vmf(mean, k, shape):
    dim = mean.shape[-1]
    shape = torch.Size(shape)
    rank = mean.ndim - 1
    if (shape[:rank] != mean.shape[:-1]) or (shape[:rank] != k.shape[:-1]):
        raise ValueError("Shape mismatch: {}, {} with target {}.".format(mean.shape, k.shape, shape))
    if k.shape[-1] != 1:
        raise ValueError("Expected single concentration for each distrsibution.")
    distribution = VonMisesFisher(mean.reshape(-1, dim), k.reshape(-1, 1))
    size = torch.Size() if len(shape) == rank else torch.Size([np.prod(shape[rank:])])
    sample = distribution.rsample(size)
    return sample.reshape(*(list(shape) + [dim]))
...
class VonMisesFisher(torch.distributions.Distribution):
  ...
  and
  https://github.com/google-research/vmf_embeddings/blob/main/vmf_embeddings/third_party/s_vae_pytorch/distributions.py
  """
