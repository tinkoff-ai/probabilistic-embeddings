...
class ShakeDropFunction(torch.autograd.Function):
    ...
    def forward(ctx, x, training=True, p_drop=0.5, alpha_range=[-1, 1]):
        if training:
            gate = torch.empty([], device=x.device, dtype=x.dtype).bernoulli_(1 - p_drop)
    ...
    def backward(ctx, grad_output):
        ...
        if gate.item() == 0:
            beta = torch.empty(grad_output.size(0), device=gate.device, dtype=gate.dtype).uniform_(0, 1)
        ...
        if residual_channel != shortcut_channel:
            padding = torch.autograd.Variable(
                torch.zeros(batch_size, residual_channel - shortcut_channel, featuremap_size[0],
                            featuremap_size[1], device=shortcut.device, dtype=shortcut.dtype))
