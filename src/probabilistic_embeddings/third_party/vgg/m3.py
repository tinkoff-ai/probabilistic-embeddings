...
class ModelM3(nn.Module):
    """
    VGG M3 model from https://github.com/ansh941/MnistSimpleCNN/blob/master/code/models/modelM3.py
    """
    def __init__(self):
    ...
    def get_logits(self, x):
        flat1 = torch.flatten(x.permute(0, 2, 3, 1), 1)
        emb = self.fc1_bn(self.fc1(flat1))
        return emb

    def forward(self, x):
        emb = self.get_embedding(x)
        logits = self.get_logits(emb)
        return logits
